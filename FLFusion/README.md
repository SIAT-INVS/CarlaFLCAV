...
Run the following command to generate the raw data. NOTE that the raw data at the road sensor is stored into "road.sensor.lidar_xxx" (the folder name can be revised).
```
...
Run the following command to fuse the maps from the road sensors and the vehicle sensors. The generated labels are then used for multi-stage training in FLPCDet.
```
