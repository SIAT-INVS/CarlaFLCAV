...
class LineMesh(object):
    ...
    def create_line_mesh(self):
        ...
        # print(line_segments_unit.shape)
...
def get_strong(bboxes_lineset):
    results = []
    for lineset in bboxes_lineset:
        try:
            points = np.asarray(lineset.points)
            lines = np.asarray(lineset.lines)
            colors = np.asarray(lineset.colors)
            if points.shape != (8,3) and lines.shape != (12,2):
                results += [lineset]
            else:
                line_mesh1 = LineMesh(points, lines, colors, radius=0.06)
                line_mesh1_geoms = line_mesh1.cylinder_segments
                results += [*line_mesh1_geoms]
