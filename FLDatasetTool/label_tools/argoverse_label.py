...
class ArgoverseLabelTool:
    ...
    def process(self):
        ...
        for index, frame in self.rawdata_df.iterrows():
            ...
            if counter == 0:
                with open(('{}/'+filename).format(self.output_dir), 'w', encoding='utf-8') as csv_file:
                    writer = csv.DictWriter(csv_file, fieldnames=self.fieldnames)
                    writer.writeheader()

            self.process_frame(index, frame, filename)

            if counter == pred_len-1: 
                file_id += 1
                counter = 0
            else:
                counter += 1
    ...
    def process_frame(self, index, frame, filename):
        ...
        for label in objects_labels:
            ...
            with open(('{}/'+filename).format(self.output_dir), 'a', encoding='utf-8') as csv_file:
                writer = csv.DictWriter(csv_file, fieldnames=self.fieldnames)
